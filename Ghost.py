...
class Ghost (Character):
    ...
    def canMove_distance(self, direction, walls):
        ...
        temp_ghost = Ghost()
        # Copy only the necessary attributes to temp_ghost
        temp_ghost.rect = self.rect.copy()
        temp_ghost.speed = self.speed
        temp_ghost.course = list(self.course)  # Copy the course list to avoid modifying the original course

        # Initialize the counter for the number of steps the ghost can move
        counter = 0
        while True:
            # Check if the ghost can move in the specified direction
            if not Character.canMove(temp_ghost, direction, walls):
                break
            # Move the ghost if possible
            Character.move(temp_ghost, direction)
            counter += 1
        return counter
